(function(window, createjs, opspark, _) {
  ...
  function update(event) {
    ...
    /*
      This code determines the movement of the computer paddle by seeing whether the ball is above or below 
      the middle of the CPU’s paddle. If the ball is above the CPU paddle’s center, it moves the paddle up. 
      If the ball is below the CPU paddle’s center, it moves the paddle down. If the hard-coded value of 14 were changed, the paddle would move more/less 
      delayed because the paddle is following the "center" of the ball. The paddle would be less delayed if the value were changed to 0 and vice versa.
    */
    ...
    if ((ball.x - ball.radius < paddlePlayer.x + widthPlayer) && (ball.y + ball.radius < paddlePlayer.y + heightPlayer) && (ball.y - ball.radius > paddlePlayer.y) && (ball.x + ball.radius > paddlePlayer.x)){
      ball.xVelocity *= -1;
    }

    if ((ball.x + ball.radius < paddleCPU.x + widthCPU) && (ball.y + ball.radius < paddleCPU.y + heightCPU) && (ball.y - ball.radius > paddleCPU.y) && (ball.x + ball.radius > paddleCPU.x)){
      ball.xVelocity *= -1;
    }
